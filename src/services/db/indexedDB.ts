...
class DatabaseService {
  ...
  async getSettings(): Promise<AppSettings> {
    try {
      ...
      // MIGRATION: Migrate deepLogicMode to 'advisory' if it is 'strict' or unset (if a migration flag isn't set)
      if (!settings._deepLogicMigratedToAdvisory) {
        if (mergedSettings.deepLogicMode === 'strict' || !mergedSettings.deepLogicMode) {
          mergedSettings.deepLogicMode = 'advisory';
        }
        mergedSettings._deepLogicMigratedToAdvisory = true;
        setTimeout(() => this.saveSettings(mergedSettings), 0);
      }
      ...
      if (mergedSettings.proxies.length === 0) {
        ...
        if (migratedProxies.length > 0) {
          ...
          mergedSettings.proxies = [];
          mergedSettings.activeProxyId = undefined;
          mergedSettings.proxyEnabled = false;
          mergedSettings.useProxyPool = false;
        }
        setTimeout(() => this.saveSettings(mergedSettings), 0);
      }
